      ...
      try {
        if (getmyData) {
          const result = await directus.request(
            readItems("following", {
              fields: [
                "follower_id.first_name",
                "follower_id.last_name",
                "follower_id.avatar",
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="flex-shrink-0">
                        <img
                          ...
                          src={`https://www.wanesni.live/assets/${user.follower_id.avatar}?access_token=${token}`}
                          alt={``}
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900">
                          {user.follower_id.first_name}{" "}
                          {user.follower_id.last_name}
                        </p>
